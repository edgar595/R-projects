...
##FLAG DOCUMENTS IDICATE IF THEY HAVE OR DON'T HAVE DOCUMENTS
...
# Define the features
domain_features <- c('AMT_CREDIT', 'AMT_GOODS_PRICE', 'DAYS_BIRTH', 'DAYS_EMPLOYED', 'AMT_INCOME_TOTAL')
...
application_train %>%
  ...
  geom_text(aes(label = sprintf("%.1f%%", percentage_contract)), 
            position = position_dodge(width = 1), 
            vjust = -0.5, size = 3) + 
  labs(title = "Distribution of defaulters by Contract Type",
       x = "Contract Type",
       fill = "TARGET") +
  scale_fill_manual(values = c("0" = "blue", "1" = "red"), labels = c("0" = "Non-default", "1" = "Default")) +
  theme_minimal() 
...
application_train %>%
  ...
  labs(title = "Distribution of TARGET by Occupation Type",
       x = "Occupation Type",
       y = "Percentage",
       fill = "TARGET") +
  scale_fill_manual(values = c("0" = "blue", "1" = "red"), labels = c("0" = "Non-default", "1" = "Default")) +
...
# Columns to drop
AMT_REQ_CREDIT <- c('AMT_REQ_CREDIT_BUREAU_HOUR', 'AMT_REQ_CREDIT_BUREAU_DAY', 'AMT_REQ_CREDIT_BUREAU_YEAR',
...
# Ensure the values are imputed
summary(application$YEARS_EMPLOYED)
...
application_train <- read.csv("data/app_train.csv")
application_test <- read.csv("data/app_test.csv")
...
# Save the rf_rs object
saveRDS(rf_rs, file = "rf_rs_model.rds")
